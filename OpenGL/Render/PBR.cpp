...
PBR::PBR(Renderer* renderer)
{
	...
	CShader * hdrCube = ResourceManager::getInstance()->LoadShader("pbr_HdrToCube", 
		"Shader\\pbr\\cube_map.vs", "Shader\\pbr\\spherical_to_cube.fs");
	CShader* integrateBrdf = ResourceManager::getInstance()->LoadShader("pbr_IntegrateBRDF",
		"Shader\\pbr\\cube_map.vs", "Shader\\pbr\\integrate_capture.fs");
	CShader* irradiance = ResourceManager::getInstance()->LoadShader("pbr_Irradiance",
		"Shader\\pbr\\cube_map.vs", "Shader\\pbr\\irradiance_capture.fs");
	CShader* prefilterCapture = ResourceManager::getInstance()->LoadShader("pbr_PrefilterCapture",
		"Shader\\scene\\screen_quad.vs", "Shader\\pbr\\prefilter_capture.fs");

	m_pbrHdrToCubeMap = ResourceManager::getInstance()->LoadMaterial("pbr_HdrToCube", hdrCube);
	m_pbrIntegrateBRDF = ResourceManager::getInstance()->LoadMaterial("pbr_IntegrateBRDF", hdrCube);
	m_pbrIrradiance = ResourceManager::getInstance()->LoadMaterial("pbr_IntegrateBRDF", irradiance);
	...
	m_renderer->Blit(m_renderTargetBRDFLut, m_pbrPrefilterCapture);
...
PBRCapture* PBR::ProcessCube(TextureCube* capture, bool prefilter)
{
	...
	m_renderer->RenderToCubeMap(m_sceneNode, capture);
