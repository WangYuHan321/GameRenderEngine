...
void DeferredRenderer::Draw(Mesh& p_mesh, uint32 drawNumObj)
{
	if (drawNumObj > 0)
	{
		...
		{
			if (drawNumObj == 1)
				glDrawElements(static_cast<GLenum>(GL_TRIANGLES), p_mesh.Indices.size(), GL_UNSIGNED_INT, nullptr);
			else
				glDrawElementsInstanced(static_cast<GLenum>(GL_TRIANGLES), p_mesh.Indices.size(), GL_UNSIGNED_INT, nullptr, drawNumObj);
		}
		else
		{
			if (drawNumObj == 1)
				glDrawArrays(static_cast<GLenum>(GL_TRIANGLES), 0, p_mesh.Positions.size());
			else
				glDrawArraysInstanced(static_cast<GLenum>(GL_TRIANGLES), 0, p_mesh.Positions.size(), drawNumObj);
		}
		p_mesh.UnBind();
	}
}