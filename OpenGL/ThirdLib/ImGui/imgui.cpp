...
//我们不使用BeginDragDropTargetCustom（）并重复其代码，因为：

//1）我们使用LastItemRectHoveredRect，它处理在代码中推送临时剪辑矩形的项目。调用BeginDragDropTargetCustom（LastItemRect）无法处理它们。

//2）而且速度更快。由于这段代码可能会被频繁调用，我们希望尽可能快地提前发布。

//还要注意HoverdWindow测试在两个函数中的定位是如何不同的（在这两种函数中，我们都针对最便宜的早期情况进行了优化）：
